...
function _tickEnemies(room) {
  ...
  for (let i = gs.enemies.length - 1; i >= 0; i--) {
    ...
    // Hit check — returns true if enemy was killed and spliced
    const killed = _checkEnemyHit(e, i, room, players);
    if (killed) continue;
...
function _checkEnemyHit(e, idx, room, players) {
  ...
  for (const b of hits) {
    e.hp     -= b.damage;
    e.flashTimer = 6;
    const bulletIdx = gs.bullets.indexOf(b);
    if (bulletIdx !== -1) gs.bullets.splice(bulletIdx, 1);
    ...
    if (e.hp <= 0) {
      ...
      return true; // Killed — skip further processing for this enemy
    }
  }
