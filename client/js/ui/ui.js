// ============================================
//   VOID SECTOR — ui.js (MULTIPLAYER)
//   Overlay UI: pause screen, wave clear
//   announcement, settings panel.
//   Reconnecting overlay moved to renderer.
//   Shop ready flags drawn by renderer.
// ============================================
...
const UI = (() => {
  ...
  function update(gameState) {
    ...
    return _inSettings
      ? _updateSettings(gameState)
      : _updatePauseMenu(gameState);
  }
  ...
  function _drawWaveAnnounce() {
    const alpha =
      Math.min(1, _waveAnnounce / 30) *
      Math.min(1, (_waveAnnounce - 20) / 30 + 1);
  ...
  function _drawPauseOverlay(gameState) {
    ...
    if (_inSettings) _drawSettings();
    else _drawPauseMenu();
  }

  function _drawPauseMenu() {
    const cy = C.CANVAS_H * 0.38;
    _tc("[ PAUSED ]", cy, C.COLOR.PRIMARY, 1, C.FONT_SIZE * 1.2, true);

    const items = ["RESUME", "SETTINGS", "QUIT TO LOBBY"];
    const lineH = CH * 1.8;
  ...
  function _drawSettings() {
    ...
    _tc("[ SETTINGS ]", cy, C.COLOR.PRIMARY, 1, C.FONT_SIZE * 1.1, true);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      _tc(
        labels[i],
        y,
        sel ? C.COLOR.PRIMARY : C.COLOR.DIM,
        sel ? 1 : 0.65,
        C.FONT_SIZE * 0.88,
        sel,
      );
  ...
  function _tc(text, y, color, alpha = 1, size = C.FONT_SIZE, bold = false) {
