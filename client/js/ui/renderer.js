...
const Renderer = (() => {
  ...
  function draw(gameState) {
    ...
    switch (gameState.screen) {
      case "LOBBY":
        break; // Lobby draws itself on top
      case "PLAYING":
        _drawPlaying(gameState);
        break;
      case "SHOP":
        _drawShop(gameState);
        break;
      case "GAMEOVER":
        _drawGameOver(gameState);
        break;
      case "DYING":
        _drawPlaying(gameState);
        break;
  ...
  function _drawPlayers(gameState) {
    ...
    for (const p of gameState.players) {
      ...
      if (p.rolling && p.rollDir) {
        ...
        _drawChar("^", p.x - p.rollDir * 1.6, p.y, C.COLOR.DIM, 0.1);
      }
  ...
  function _drawEnemies(gameState) {
    for (const e of gameState.enemies) {
      ...
      const color =
        e.flashTimer > 0 && Math.floor(e.flashTimer / 2) % 2 === 0
          ? C.COLOR.WHITE
          : def.COLOR;
  ...
  function _drawGameOver(gameState) {
    ...
    _drawTextCentered(
      "WAVE " + (gameState.finalStats?.wave || gameState.wave),
      C.CANVAS_H * 0.3,
      C.COLOR.DIM,
      0.8,
      C.FONT_SIZE * 0.9,
